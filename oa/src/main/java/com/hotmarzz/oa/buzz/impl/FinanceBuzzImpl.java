...
public class FinanceBuzzImpl implements FinanceBuzz {
	...
	public void add(CampusWaterDto cw) throws Exception {
		// 默认为登录人所在校区
		cw.setSchoolIdDto(((Emp) (session
				.getAttribute(SessionUtils.LOGIN_EMP_KEY))).getSchoolDistrict()
				.getSchoolId());
		// 默认流水批次为当前月份
		...
		// 默认经手人为登录人
		cw.setBrokerage(((Emp) (session
				.getAttribute(SessionUtils.LOGIN_EMP_KEY))).getEmpName());
		cw.setCreateUser(((Emp) (session
				.getAttribute(SessionUtils.LOGIN_EMP_KEY))).getEmpName());
		finDao.add(cw);
	}
	...
	public void addFin(Financial fin) throws Exception {
		// 申请人
		fin.setApplyUser(((Emp) (session
				.getAttribute(SessionUtils.LOGIN_EMP_KEY))).getEmpName());
		// 创建人
		fin.setCreateUser(((Emp) (session
				.getAttribute(SessionUtils.LOGIN_EMP_KEY))).getEmpName());
		// 申请状态
		fin.setFinStatus("未审核");
		//添加校区名
		fin.setSchoolName(((Emp) (session
				.getAttribute(SessionUtils.LOGIN_EMP_KEY))).getSchoolDistrict().getSchoolName());
	...
	public void updateSP(Financial fin) throws Exception {
		Emp emp = (Emp) session.getAttribute(SessionUtils.LOGIN_EMP_KEY);
		fin.setApproveUser(emp.getEmpName());
		finDao.updateSP(fin);
	}
